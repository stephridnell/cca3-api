...
const getPokemon = async (pokemonId) => {
  var params = {
    KeyConditionExpression: 'PK = :pid AND SK = :pid',
    ExpressionAttributeValues: {
      ':pid': { N: pokemonId },
    },
    TableName: process.env.POKEMON_TABLE,
  ...
  const command = new QueryCommand(params)
  const response = await ddbClient.send(command)
  const pokemonData = response.Items?.[0]
  if (!pokemonData) {
    return undefined
  }

  const unmarshalledPokemon = unmarshall(pokemonData)

  return {
    name: unmarshalledPokemon.name,
    imageUrl: unmarshalledPokemon.imageUrl,
  }
}
...
exports.getUserGames = async (userId) => {
  var params = {
    KeyConditionExpression: "PK = :uid AND begins_with(SK, :gamePrefix)",
    ExpressionAttributeValues: {
      ':uid': { S: userId },
      ':gamePrefix': { S: 'g#' }
    },
